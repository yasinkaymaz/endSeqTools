...
#$srcDIR/GTF_parser_For_GeneAnnotation.sh $GeneAnnotationFile $GenesFile
else
...
$srcDIR/GTF_parser_For_GeneSpace.sh $GeneAnnotationFile $GeneSpaceFile
$srcDIR/GTF_parser_For_GeneAnnotation.sh $GeneAnnotationFile $GenesFile
...
Rscript $srcDIR/ppois.R $WORKINGDIR/PApool.bed_sorted_lambda.bed.tmp $WORKINGDIR/PApool.bed_sorted_lambda_ppois.bed.tmp
echo "Filtration is complete on Pooled data!"

echo "Now clustering closer genomic locations withing 100nt to merge expression values..."

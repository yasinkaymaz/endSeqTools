...
#  utrs <- unique(data[which(data[,5] == i  ),5])
  for (u in utrs){
    u.data <- data[which(data[,5] == u  ),]
    if(u.data$V6[1] == "-"){
      u.data$PAS.dist <- u.data$V3 - (u.data$V8 +(u.data$V9-u.data$V8)/2)
      u.data$Site <- c("Distal",rep("Middle",nPA-2),"Proximal")
    }else{
      u.data$PAS.dist <- (u.data$V8 +(u.data$V9-u.data$V8)/2)-u.data$V2
      u.data$Site <- c("Proximal",rep("Middle",nPA-2),"Distal")
    }
    u.data$C1mean <- rowMeans(data[which(data[,5] == u  ),14:(13+NoC1)])
    u.data$C2mean <- rowMeans(data[which(data[,5] == u  ),(14+NoC1):(13+NoS)])
    ...
    utr.len.cor <- cor(u.data$C1utrlen,u.data$C2utrlen)
      ...
      colnames(plot.data) <- c("Type","Proximal",rep("Middle",nPA-2),"Distal")
